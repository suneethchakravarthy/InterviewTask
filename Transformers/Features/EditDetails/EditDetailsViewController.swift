...
class EditDetailsViewController: BaseViewController {
    @IBOutlet weak var mainTableView: UITableView!

    // MARK: - Parameters
    var viewModel: EditDetailsViewModel? {
        didSet {
    ...
    @objc func saveButtonClicked() {
        closeKeyboard()
        viewModel?.updateTransformerDetails()
    }
}
extension EditDetailsViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, heightForFooterInSection section: Int) -> CGFloat {
        return CGFloat(viewModel?.sectionFooterViewHeight ?? 0.0)
    }
    func tableView(_ tableView: UITableView, heightForHeaderInSection section: Int) -> CGFloat {
        return CGFloat(viewModel?.sectionHeaderViewHeight ?? 0.0)
    }
    ...
    func tableView(_ tableView: UITableView, viewForFooterInSection section: Int) -> UIView? {
        ...
        footerView.customButton.setTitle("Save", for: .normal)
        footerView.delegate = self
        return footerView
    }
}
extension EditDetailsViewController: CustomHeaderDelegate {
    func buttonTapped() {
        viewModel?.updateTransformerDetails()
    }
}
...
extension EditDetailsViewController {
        private func bindViewModel() {
            viewModel?.didItemUpdate.bind{[weak self]  in
                guard $0 else { return }
...
extension EditDetailsViewController: UIPickerViewDelegate, UIPickerViewDataSource {
